...
public class FavoriteUserDBHelper {
    public static final String TAG = "FavoriteUserDBHelper";
    ...
    public static ArrayList<FavoriteUserDto> getFavoriteByGroup(long groupNumber) {
        ...
        Cursor cursor = resolver.query(AppContentProvider.GET_FAVORITE_USER_CONTENT_URI, columns, GROUP_NO + "=" + groupNumber + " AND " + IS_TOP + " = 0", null, null);
    ...
    public static ArrayList<FavoriteUserDto> getFavoriteTop() {
        String[] columns = new String[]{"*"};
        ContentResolver resolver = CrewChatApplication.getInstance().getApplicationContext().getContentResolver();
        Cursor cursor = resolver.query(AppContentProvider.GET_FAVORITE_USER_CONTENT_URI, columns, IS_TOP + " = " + 1, null, null);
    ...
    public static boolean isExist(FavoriteUserDto userDto) {
        String[] columns = new String[]{"*"};
        ContentResolver resolver = CrewChatApplication.getInstance().getApplicationContext().getContentResolver();
        Cursor cursor = resolver.query(AppContentProvider.GET_FAVORITE_USER_CONTENT_URI, columns, USER_NO + "=" + userDto.getUserNo(), null, null);
    ...
    public static FavoriteUserDto isFavoriteUser(long userId) {
        String[] columns = new String[]{"*"};
        ContentResolver resolver = CrewChatApplication.getInstance().getApplicationContext().getContentResolver();
        Cursor cursor = resolver.query(AppContentProvider.GET_FAVORITE_USER_CONTENT_URI, columns, USER_NO + "=" + userId, null, null);
...
//                    Log.d(TAG, new Gson().toJson(user));
                    ...
                    values.put(MOD_DATE, user.getModDate());
                    values.put(IS_TOP, user.getIsTop());
                    Log.d("sssDebug", "addusser" + user.getUserNo());
                    ContentResolver resolver = CrewChatApplication.getInstance().getApplicationContext().getContentResolver();
    ...
    public static boolean isExistByKey(FavoriteUserDto userDto) {
        String[] columns = new String[]{"*"};
        ContentResolver resolver = CrewChatApplication.getInstance().getApplicationContext().getContentResolver();
        Cursor cursor = resolver.query(AppContentProvider.GET_FAVORITE_USER_CONTENT_URI, columns, USER_NO + "=" + userDto.getUserNo() + " AND " + GROUP_NO + "=" + userDto.getGroupNo(), null, null);
