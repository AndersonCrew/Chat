...
public class ChatMessageDBHelper {
    ...
    public static boolean isExist(ChattingDto dto) {
        ...
        Cursor cursor = resolver.query(AppContentProvider.GET_MESSAGE_CONTENT_URI, columns, MESSAGE_NO + "=" + dto.getMessageNo(), null, null);
    ...
    public static boolean deleteMessageByLocalRoomNo(long roomNo) {
        try {
            ContentResolver resolver = CrewChatApplication.getInstance()
                    .getApplicationContext().getContentResolver();
            resolver.delete(AppContentProvider.GET_MESSAGE_CONTENT_URI, ROOM_NO + "=" + roomNo, null);
            return true;
...
//        if (CrewChatApplication.CrewChatLocalDatabase && dto.getMessageNo() != 262530 && dto.getMessageNo() != 262529) {// for test
    ...
    public static ChattingDto addMsg(Cursor cursor) {
    ...
    public static List<ChattingDto> getMsgSession(long roomNo, long baseMsgNo, int type) {
        ...
        if (cursor != null) {
            if (cursor.getCount() > 0) {
                try {
                    cursor.moveToLast();
                    ChattingDto obj = addMsg(cursor);
                    if (obj.getMessageNo() == Long.MAX_VALUE && obj.isHasSent()) {
                    } else {
                        mesArray.add(obj);
                    }
                    while (cursor.moveToPrevious()) {
                        ChattingDto dto = addMsg(cursor);
                        if (dto.getMessageNo() == Long.MAX_VALUE && dto.isHasSent()) {
                        } else {
                            mesArray.add(dto);
                        }
                    }
